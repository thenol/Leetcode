...
* **方法论(思路)**：本质是有向无环图(也就是无后效性)
    ...
    1. **确定状态** -> 聚焦 **```单个元素```** （**研究最优策略的最后一步**），同时关注数据规模，规模也是提示，可以决定状态中所含变量个数，**```状态的内涵或者必要条件，必须能够正确表示题目里面的所有关系、针对每一个元素时的决策，以及最终解```**（参见**87. 扰乱字符串.py**）
        ...
        * **```状态的完备性```**：对应解的完备性，其必然记录了所有的可能性(用于求得最优解，最优解的必要性)，也就是暴力情况下的可能性，它一定是所有情况下的最优解(一定可以通过可能性展开，从而找到相互之间关系，避免重复计算，即空间换时间)
...
* **两种写法**：记忆化搜索dfs(注意可以**挂或者传入缓存表，如果需要预处理就必须传入预处理之后的缓存表**)、刷表法dp
    ...
    * 刷表法dp
        * 好处：
            ...
            * 代码行数少，尤其状态数少于2种的时候
        * 坏处：
            * 需要严格以来表格之间位置关系，因此需要显示考虑表格填写顺序，需要处理细节
            * **对于状态高于3种的情况，即3纬以上的状态数组，计算顺序较为复杂，不好具像化，通过迭代的方式构造计算顺序较为耗时**
        * 填表法技巧：一定要用特列法，先画一个简单的表格，摸清表格之间以来关系，再去写遍历
    * **最佳实践✅**：
        * **当状态数组维度不高于2时，推荐采用刷表法**，即迭代法，此时计算关系简单，容易构造，且代码简介
        * **当状态数组维度高于2时，推荐使用挂表法，即递归法**，此时状态之间依赖关系较为复杂，对于高纬度状态之间的计算顺序，通过迭代来构造比较耗时，且容易犯错，因此挂表法是最优选择
