...
class Solution:
    # 方法不太好，计算复杂度比较高，讨论了很多不成立的情况，即当字符数量超过2个的时候，就一定不成立了
    ...
    def numDecodings(self, s: str) -> int:
        ...
        for i in range(len(ascii_uppercase)):
            mapper[str(i+1)] = ascii_uppercase[i] # 从"1"开始啊，哥哥❗️❗️❗️❗️❗️❗️❗️❗️
        
        # state: d[i]表示s[:i]范围可以生成的方法数
        ...
        def f(i):
            ...
            # transition
            ans = 0
            if 0<=i-1 and s[i-1] in mapper: # ⭕️：范围判断不能漏啊
                ans += f(i-1)*1
            
            if 0<=i-2 and s[i-2:i] in mapper:
                ans += f(i-2)*1
            return ans
        return f(len(s))