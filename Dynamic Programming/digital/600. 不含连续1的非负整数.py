...
class Solution:
    def findIntegers_1(self, n: int) -> int:
        ...
        def f(i, pre, is_limit):
            """
            表示当前进行到了第i位，前面是否有1，当前是否受限的个数；
            0<=i<=n，本质是类似于哨兵，例如当n=10(0b1010)时，在前面加了一个0，即0b01010，来计算个数，此时不影响最终结果
            """
            if i < 0: # 本质当i==0的时候，也就是调用f(-1,d == 1,is_limit and d == up)，即右移0位的时候，判断最低位，如果符合条件，就算一次
                return 1
