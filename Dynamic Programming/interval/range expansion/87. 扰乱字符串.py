...
"""
状态确定：
1. 最后一步：必然 s1=x + y 和 s2=x1 + y1 全部匹配，即s1=f(x) + f(y) 和 s2匹配
    <= 如果s1中的 y 和 s2 中的 y1匹配，则只需扰乱 x，看能否成x1即可，当然，也有可能 x和y1匹配
    <= 因此决策中，需要考虑字符串匹配问题，所以 f(i, j, len)必须包含这三个变量，即 s1的末尾位置i，s2的末尾位置，以及往前 len 个字符是否可以扰乱
    <= 状态检验 -> 可以满足所有决策关系，
    <= 状态可以用 d[i][j][len] 来表达，当然也可以用 四维 来表达
