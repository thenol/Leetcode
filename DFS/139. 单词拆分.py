...
class Solution:
    ...
    # 完全背包
    # ❌ WA；原因：放入背包的必须有顺序，即按照顺序放入背包
    def wordBreak_1(self, s: str, wordDict: List[str]) -> bool:
        # state: d[i][j]表示wordDict[:i]范围内是否能表示s[:j]
        # 0<=i<=N; 0<=j<=M
        N, M = len(wordDict), len(s)

        @cache
        def f(i, j):
            """表示wordDict[:i]范围内是否能表示s[:j]"""
            nonlocal N, M, s, wordDict
            if j==0: return True
            if i<=0 or j<0: return False

            ans = f(i-1, j) # 当前不选
            if s[:j].endswith(wordDict[i-1]): # ❌有序放入背包的情况下，如果当前没选上，后续也就不可能再选上了
                ans = ans or f(i, j-len(wordDict[i-1])) # 选完可以接着选；
            return ans
